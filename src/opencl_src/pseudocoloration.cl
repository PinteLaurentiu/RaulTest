...
__kernel void watershedPseudocoloration(__global unsigned char* in,
        __global int* labels,
        __global int* colors,
        int colorCount,
        __global unsigned char* out) {
    unsigned long g_id = get_global_id(0);
    int in_value = colors[labels[g_id]] * 1276 / (colorCount - 1);
    int out_valueR, out_valueG, out_valueB;
    if (in_value < 256) {
        out_valueR = 255 - in_value;
        out_valueG = 0;
    ...
    out[g_id * 3] = (in[g_id] * 4 + out_valueR) / 5;
    out[g_id * 3 + 1] = (in[g_id] * 4 + out_valueG) / 5;
    out[g_id * 3 + 2] = (in[g_id] * 4 + out_valueB) / 5;
}